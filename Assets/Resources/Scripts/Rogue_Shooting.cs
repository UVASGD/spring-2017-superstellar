...
public class Rogue_Shooting : Photon.MonoBehaviour {
	...
	private int starPointNum = 3;
	// how many points the star has

	private int maxPointHealth = 20;
	...
	private List<Vector2> pointVectList = new List<Vector2>(3);
	// the direction vectors for projectiles

	private List<float> pointAngles = new List<float>(3);
	// the angles at which starpoints are shot

	private List<float> pointAngles2 = new List<float>(3);
	...
	void Start() {
		...
		canShoot = new List<int>{ 1, 1,1};

		starMat = Resources.Load<Material> ("Materials/AI_Rogue");
	...
	void resetShooting(Quaternion q, int numPoints){
		...
		pointAngles2.Add (topAngle2 + 120);
		pointAngles2.Add (topAngle2 + 240);
	...
	void upgradeStar(){
		...
		resetShooting (transform.rotation, 3);
	...
	void rotate() {
		...
		Vector3 pos = new Vector3(-10f,-10f,0f);
		Vector3 dir = pos - transform.position;
